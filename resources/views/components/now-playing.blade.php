 <div class="swiper-slide !w-fit">
     <a href="{{ route('films.show', $playing->id) }}" class="card">
         <div
             ...
             <div class="flex w-full aspect-[2/3] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                 <img src="{{ $playing->getFirstMediaUrl('film') }}" class="w-full h-full object-cover"
                     alt="{{ $playing->name }}">
             </div>
             <div class="flex flex-col gap-3">
                 <h3 class="font-semibold text-lg text-center line-clamp-2 min-h-[56px] mb-3">
                 ...
                 <div class="flex items-center gap-[6px] min-h-[40px]">
                     ...
                     <p class="text-sm text-ngekos-grey dark:text-[#99a1af] line-clamp-2">Producer:
                         {{ $playing->producer }}</p>
                 </div>
             </div>
         </div>
